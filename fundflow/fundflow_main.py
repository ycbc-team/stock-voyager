#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""fundflow 页面总控：先生成 JSON，再渲染 HTML（A股 / 港股 共用）。"""
...
def build_fundflow_report(data_date: str | None = None, out_dir: str | None = None, topn: int = 10, verbose: bool = True, market: str = "ashare") -> Dict:
    ...
    json_path = write_report_json(result, out_dir=data_dir, market=market)
    html_path = os.path.join(site_dir, html_name)
    payload = read_json(json_path)
    write_html(html_path, payload.get("data") if isinstance(payload, dict) and "data" in payload else payload, market=market)
